...
export function loadLookup() {
  return pending ||= fetch('/lexical/dodson-2010-v3/lookup.json').then(async response => {
    if (!response.ok) throw new Error('Lexicon could not be loaded.');
    const data = await response.json() as LookupBundle;
    if (data.releaseId !== 'dodson-2010-v3' || !data.entries || !data.links) throw new Error('Lexicon release mismatch.');
    return data;
