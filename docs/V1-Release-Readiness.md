...
- macOS public distribution requires Developer ID signing and notarization; Apple login/2FA will require owner participation. Microsoft Azure Artifact Signing Basic is the selected Windows-signing path, subject to identity validation and the owner's explicit approval immediately before starting its paid subscription. Windows remains beta until signing is configured and verified.
