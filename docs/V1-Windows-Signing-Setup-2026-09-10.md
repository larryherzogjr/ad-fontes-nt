...
Microsoft Azure Artifact Signing is configured for the Windows x64 public-release path. A signed release-candidate package has been built and independently checked against its recorded hashes; acceptance on Windows 11 remains open.
...
- Run `34560757315` passed every shared and Windows step at source `951f456a43915fe9c46bb4f02da21bd1f6553da0`. The application and final installer both passed SignTool verification with zero warnings and errors; the chain was issued to Larry Herzog Jr. and the RFC 3161 timestamp verified. GitHub uploaded `Ad-Fontes-NT-Windows-x64-signed-1.0.0-rc.2` with artifact digest `sha256:c680e7a61939f72eff9dcd6ee7c35cdd5cdd9ba8d588ed300edd23df1d3f07a5`.
...
`npm run test:desktop` passed all five desktop groups after the workflow change; the complete 53 Node tests and 19 Python tests also passed. The workflow YAML parsed locally and `git diff --check` passed. The expected five GitHub secret names are present; secret values cannot be read back and were not exposed. The final private artifact was downloaded to the ignored local artifact directory. After normalizing its Windows CRLF checksum-file endings for macOS verification, both entries matched: installer SHA-256 `59338f9f4fbdb22551e1ff01e6929fec98832c3664fe7aa5d7de103da448161d` and updater-signature SHA-256 `388d6b401aa6736d508c46667e1a73ec8964400c00249b2bbd84e0db4ad6b833`. `BUILD-INFO.json` independently records the same source, version, release and hashes.

Still required: install this exact package on Windows 11 x64; verify the displayed publisher and installed application signature, offline reading and uninstall; then rebuild both supported platforms at one new immutable version for the served Windows updater rehearsal and upgrade test. The already-published macOS-only `1.0.0-rc.2` directory remains immutable.
