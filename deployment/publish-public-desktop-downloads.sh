#!/usr/bin/env bash
# Publish the approved RC6 installers behind the public /downloads page.
...
mac_name=Ad-Fontes-NT-macOS-Apple-Silicon-1.0.0-rc.6.dmg
windows_name=Ad-Fontes-NT-Windows-x64-1.0.0-rc.6.exe
windows_source=/var/www/ad-fontes-updates/releases/1.0.0-rc.6/ad-fontes-nt-1.0.0-rc.6-windows-x86_64.exe
...
[[ $(sha256sum "$staging/$mac_name" | cut -d ' ' -f 1) == 881f394d11d811b1646db61ddbdaabf0b1279034f086b0586a89571ca31779e6 ]] || { echo 'Staged macOS DMG checksum mismatch.' >&2; exit 1; }
[[ -f "$windows_source" ]] || { echo 'The published Windows RC6 installer is missing.' >&2; exit 1; }
[[ $(sha256sum "$windows_source" | cut -d ' ' -f 1) == 097aba981ba29e97691a553ab90f3cce8369425311cd4c9736f4fe7972b4e508 ]] || { echo 'Published Windows RC6 checksum mismatch.' >&2; exit 1; }
