#!/usr/bin/env bash
# Publish the approved 1.1.3 installers behind the public /downloads page.
...
mac_name=Ad-Fontes-NT-macOS-Apple-Silicon-1.1.3.dmg
windows_name=Ad-Fontes-NT-Windows-x64-1.1.3.exe
windows_source=/var/www/ad-fontes-updates/releases/1.1.3/ad-fontes-nt-1.1.3-windows-x86_64.exe
...
[[ $(sha256sum "$staging/$mac_name" | cut -d ' ' -f 1) == afc31422ff28d6d9fddaf7e1c6d4fac937ec8f32b893e78713eebdf78cdc73f7 ]] || { echo 'Staged macOS DMG checksum mismatch.' >&2; exit 1; }
[[ -f "$windows_source" ]] || { echo 'The published Windows 1.1.3 installer is missing.' >&2; exit 1; }
[[ $(sha256sum "$windows_source" | cut -d ' ' -f 1) == 5a70b41a2126cce33cecabc8f39d3c0e772ad0e3253982251966e826370b757a ]] || { echo 'Published Windows 1.1.3 checksum mismatch.' >&2; exit 1; }
