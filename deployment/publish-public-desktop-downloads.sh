#!/usr/bin/env bash
# Publish the approved RC3 installers behind the public /downloads page.
...
mac_name=Ad-Fontes-NT-macOS-Apple-Silicon-1.0.0-rc.3.dmg
windows_name=Ad-Fontes-NT-Windows-x64-1.0.0-rc.3.exe
windows_source=/var/www/ad-fontes-updates/releases/1.0.0-rc.3/ad-fontes-nt-1.0.0-rc.3-windows-x86_64.exe
...
[[ $(sha256sum "$staging/$mac_name" | cut -d ' ' -f 1) == 7e0a75293e9f8fc69f4ed44dab51abb78caf92856ad9ae7fb89e94310e9bc61e ]] || { echo 'Staged macOS DMG checksum mismatch.' >&2; exit 1; }
[[ -f "$windows_source" ]] || { echo 'The published Windows RC3 installer is missing.' >&2; exit 1; }
[[ $(sha256sum "$windows_source" | cut -d ' ' -f 1) == cbd6dc296dfcdcc9422878a91c07a26b94961932b7bda3fee89d3896d3676b64 ]] || { echo 'Published Windows RC3 checksum mismatch.' >&2; exit 1; }
