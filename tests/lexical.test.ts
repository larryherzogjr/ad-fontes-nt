...
const data = JSON.parse(readFileSync('app/public/lexical/dodson-2010-v1/lookup.json', 'utf8')) as LookupBundle;
test('Dodson reproduces pinned source and output checksums offline', () => {
  execFileSync('python3', ['scripts/import_lexicon.py']);
  const manifest = JSON.parse(readFileSync('sources/dodson/manifest.json', 'utf8'));
  ...
  assert.equal(createHash('sha256').update(readFileSync('app/public/lexical/dodson-2010-v1/lookup.json')).digest('hex'), manifest.outputSha256);
});
...
test('Word Explorer targets preserve the published metadata and access labels', () => {
  const source = JSON.parse(readFileSync('sources/word-explorer/words.json','utf8'));
