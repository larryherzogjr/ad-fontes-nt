...
test('public desktop download page exposes only the approved RC5 installers', async () => {
  const page = await readFile('app/app/downloads/page.tsx', 'utf8');
  assert.match(page, /Desktop release candidate/);
  assert.match(page, /1\.0\.0-rc\.5/);
  assert.match(page, /Ad-Fontes-NT-macOS-Apple-Silicon-1\.0\.0-rc\.5\.dmg/);
  assert.match(page, /Ad-Fontes-NT-Windows-x64-1\.0\.0-rc\.5\.exe/);
...
test('public project status distinguishes live RC5 from final v1.0', async () => {
  const reader = await readFile('app/reader/reader.tsx', 'utf8');
  assert.match(reader, /public release-candidate status/);
  assert.match(reader, /Version 1\.0\.0-rc\.5 is the current cross-platform desktop\s+candidate/);
