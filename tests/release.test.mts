...
test('public desktop download page exposes only the RC9 candidate installers', async () => {
  const page = await readFile('app/app/downloads/page.tsx', 'utf8');
  assert.match(page, /Desktop release candidate/);
  assert.match(page, /1\.0\.0-rc\.9/);
  assert.match(page, /Ad-Fontes-NT-macOS-Apple-Silicon-1\.0\.0-rc\.9\.dmg/);
  assert.match(page, /Ad-Fontes-NT-Windows-x64-1\.0\.0-rc\.9\.exe/);
...
test('public project status distinguishes RC9 from final v1.0', async () => {
  ...
  assert.match(reader, /Version 1\.0\.0-rc\.9 is the current cross-platform desktop\s+candidate/);
