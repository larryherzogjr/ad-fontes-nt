...
test('public desktop download page exposes only the RC11 candidate installers', async () => {
  const page = await readFile('app/app/downloads/page.tsx', 'utf8');
  assert.match(page, /Desktop release candidate/);
  assert.match(page, /1\.0\.0-rc\.11/);
  assert.match(page, /Ad-Fontes-NT-macOS-Apple-Silicon-1\.0\.0-rc\.11\.dmg/);
  assert.match(page, /Ad-Fontes-NT-Windows-x64-1\.0\.0-rc\.11\.exe/);
...
test('public project status distinguishes RC11 from final v1.0', async () => {
  ...
  assert.match(reader, /39 reviewed\s+comparison notes/);
  assert.doesNotMatch(reader, /30 reviewed comparison notes/);
  assert.match(reader, /Version 1\.0\.0-rc\.11 is the current cross-platform desktop\s+candidate/);
